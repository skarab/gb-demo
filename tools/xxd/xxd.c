...
char *pname;
{
  ...
  fprintf(stderr, "    -w          convert 8 to 4 bits for GBDK and others...\n");
  exit(1);
}
  ...
  while (argc >= 2)
    {
	  ...
	  else if (!STRNCMP(pp, "-w", 2)) oldschool = 1;
	  else if (!STRNCMP(pp, "-x", 2)) rawpict = 1;
  ...
  if (hextype == HEX_CINCLUDE)
    {
      if (fp != stdin)
	{
	  fprintf(fpo, "const unsigned char %s", isdigit((int)argv[1][0]) ? "__" : "");
